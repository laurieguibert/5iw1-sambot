        ...
        database_connection.connect(function(err) {
            ...
            database_connection.query("SELECT hash FROM tokens WHERE code = '"+results.response+"'", function (err, result, fields) {
                ...
                if (result.length > 0) {
                    ...
                    session.send("Your code looks to be wrong, please try with an other code");
                }
            });
        });
    } 
...
bot.dialog('Events', [
    function (session, args, next) {
        if (session.userData.token) {
            ...
            axios.get(start_url + 'events/search/?token=' +  session.userData.token + "&sort_by=date" + event_place + event_keyword)
            .then(response => {
...
bot.dialog('Categories', [
    function (session, args) {
        if (session.userData.token) {
            axios.get(start_url + 'categories/?token=' +  session.userData.token)
            .then(response => {
    ...
    function (session, results) {
        if (results.response.entity) {
            ...
            axios.get(start_url + 'events/search/?token=' +  session.userData.token + "&categories=" + category.id)
            .then(response => {
                create_event_card(session, response);
            })
            .catch(error => {
